...
# df_clonotype2[grepl("TRAV11",df_clonotype2$chain1_v_gene) & grepl("TRAJ18",df_clonotype2$chain1_j_gene),]$canonical = "canonical"
...
df_freq = ComputeFreq(mat_cdr3_pair,
                      ...
                      ident,
                      canonicalList)
df_freq=df_freq[order(rownames(df_freq)),]
df_freq[,c("cano_pct", "noncano_pct")]
df_freq$cluster = rownames(df_freq)
ggplot(df_freq, aes(x=cluster,y=noncano_pct)) +
  geom_point() 
...
new_ident = factor(new_ident, levels=c("NKTp","NKT1","NKT2","NKT17"))
df_freq = ComputeFreq(mat_cdr3_pair,
                      df_clonotype_labeled,
                      new_ident,
                      canonicalList)
...
ggplot(df_freq, aes(x=factor(cluster,levels=c("NKTp","NKT1","NKT2","NKT17")),y=noncano_pct*100)) +
  geom_point(size=7) +
  geom_line(group = 0,size=2) +
  xlab("") +
  ylab("% of non-canonical TCRs") +
  ylim(0,50) +
  ggsave("NKT_noncanonical_frequency.pdf",width=7,height=7)
dev.off()
