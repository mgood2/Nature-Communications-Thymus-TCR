...
for(i in 1:length(long_cellcount_clonotypes$cell)){
  long_cellcount_clonotypes$cluster[i] = ident_MAIT[as.character(long_cellcount_clonotypes$cell[i])]
}
library(dplyr)
...
ggplot(summary_df, aes(x = factor(clonotype, levels=unique(summary_df$clonotype)
  ...
  # scale_fill_manual(values=c("purple","black","darkgreen","black","black","black","purple","blue","blue")) +
  ...
  ggsave("Figure6B_bargraph_clonotype_MAIT.pdf", width = 20)
dev.off()
