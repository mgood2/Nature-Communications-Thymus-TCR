...
df_clonotype2[!grepl(chain1_v_gene,df_clonotype2$chain1_v_gene) | !grepl(chain1_j_gene,df_clonotype2$chain1_j_gene),]$selected = TRUE
...
seurat.MAIT.removed221 = subset(seurat.MAIT_v3, cells =  colnames(seurat.MAIT@scale.data)[!(colnames(seurat.MAIT@scale.data) %in% selected_ind)])
cellind_removed_221cells = colnames(seurat.MAIT.removed221)
save(cellind_removed_221cells,file="cellind_removed_221cells.RData")

sset_MAIT2 = ImportSeurat(sset,seurat.MAIT.removed221, version="v3")

MAIT_cdr3_mat2 = SetCDR3Mat(sset_MAIT2, cell_ind = colnames(seurat.MAIT.removed221),
                           chain1 = c("TRA"), chain2 = c("TRB"))
...
ident = as.factor(seurat.MAIT.removed221@active.ident)
...
plt4 = PlotFrequency(MAIT_cdr3_pair_mat2, ident = seurat.MAIT.removed221@active.ident, reverse = TRUE, selected_ind = NULL,
...
plt4 = plt4 + theme_bw() + ylab("") +
  ...
  theme(
    ...
    legend.position = "none") + ggsave("MAIT_noncano_freq_alphachain_removed221cells.pdf",width=7,height=7)
