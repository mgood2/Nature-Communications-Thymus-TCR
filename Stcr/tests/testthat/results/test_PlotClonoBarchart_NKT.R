...
plt = PlotClonotypes(NKT_cdr3_pair_mat, new_ident, cellMinLimit = 1)
print(plt)
...
cellcount_clonotypes = colSums(mat_cdr3_pair_NKT2)
...
long_cellcount_clonotypes =  reshape2::melt(mat_cdr3_pair_NKT2[,rownames(sub_df)])
...
for(i in 1:length(long_cellcount_clonotypes$cell)){
  long_cellcount_clonotypes$cluster[i] = new_ident[as.character(long_cellcount_clonotypes$cell[i])]
}
library(dplyr)
...
ggplot(summary_df, aes(x = factor(clonotype, levels=unique(summary_df$clonotype)
  ...
  ggsave("Figure7B_bargraph_clonotype_NKT.pdf", width = 20)
dev.off()
